...
* 使用房间列表查询是否已经有可用房间。如果没有的话，赶紧呼朋唤友准备开始吧
* 要加入准备中的房间，使用加入房间 ID，如ddz.join 1
* 要创建房间，使用创建房间。添加参数-m以指定模式。0：经典模式，1：魔改模式，2：万宁模式。如ddz.create -m 1
* 当人满后，房主使用开始，开始游戏
* 私聊机器人，使用查看手牌 或 手牌 以获取手牌等信息
* 使用出牌 或 出 以出牌，输入出牌 过 / 出 过 则可以直接跳过本轮
* 使用退出房间退出当前房间。若房主退出，则房主身份会顺延到下一位玩家
* 使用解散房间解散当前房间（仅房主可用）
* 当出现问题时，使用重置斗地主房间以重置数据表
